...
export default function SignInPageContent() {
  ...
  const userType = searchParams.get('user_type') // 'customer' or 'supplier'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Handle based on intended user type
      if (userType === 'customer') {
        await handleCustomerFlow(user)
      } else {
        await handleSupplierFlow(user)
      }
        ...
        <Card className="w-full max-w-md shadow-xl border-gray-200">
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold text-primary-700 pt-10">
              {userType === 'customer' ? 'Welcome!' : 'Welcome Back!'}
            </CardTitle>
            <CardDescription className="text-gray-600">
              {userType === 'customer' 
                ? 'Sign in to send enquiries and manage your party'
          ...
          <CardFooter className="flex flex-col items-center space-y-4 pt-6">
            ...
            <p className="mt-6 text-center text-sm text-gray-600 pb-10">
              ...
              <Link
                href={userType === 'customer' ? '/sign-up' : '/suppliers/onboarding'}
