  ...
  const handleSave = async () => {
    try {
      ...
      const availabilityData = {
        ...
        weekendPremium: weekendPremium, // ADD THIS LINE
      }
          ...
          <Card className="border-blue-200 bg-blue-50">
            <CardContent className="p-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-blue-900 mb-2">{config?.name || "Time Slot Based"} System</h3>
                  <p className="text-sm text-blue-800 mb-3">
                    {config?.description || "Availability based on specific time slots and working hours"}
                  </p>
                  <div className="text-sm text-blue-700">
                    <p className="font-medium mb-1">Perfect for:</p>
                    <p>
                      {config?.suitableFor ||
                        "Services requiring specific appointment times (venues, entertainers, face painters)"}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <Tabs defaultValue="hours" className="w-full">
          ...
          <TabsContent value="hours" className="p-3 sm:p-6 space-y-4 sm:space-y-6">
            <div className="space-y-4 sm:space-y-6">
              <div>
                <h3 className="text-base sm:text-xl font-semibold mb-2 text-gray-900">Weekly Time Slot Schedule</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-6">
                  ...
                  >
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-500">(9am-5pm, Mon-Fri)</div>
                    </div>
                  </Button>
                  <Button
                    variant="outline"
                  ...
                  >
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-500">(Sat-Sun Only)</div>
                    </div>
                  </Button>
                  <Button
                    variant="outline"
                  ...
                  >
                    <div className="text-center">
                      ...
                      <div className="text-xs text-gray-500">(8am-8pm, All Days)</div>
                    </div>
                  </Button>
                </div>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 sm:p-6 rounded-lg border transition-all ${
                        dayData.active
                          ? "bg-white border-[hsl(var(--primary-200))] shadow-sm"
                    ...
                    >
                      <div className="flex items-center gap-3 mb-3 sm:mb-4">
                        ...
                        <Label
                          htmlFor={`day-${day}`}
                          className="text-sm sm:text-base font-medium capitalize cursor-pointer flex-1"
                        >
                          {day}
                        </Label>
                        <IconComponent
                          className={`w-4 h-4 sm:w-5 sm:h-5 ${dayData.active ? "text-green-600" : "text-gray-400"}`}
                        />
                      </div>

                      {dayData.active && (
                        <div className="space-y-3 sm:space-y-4">
                          {Object.entries(TIME_SLOTS).map(([slotId, slotConfig]) => {
                            const slotData = dayData.timeSlots?.[slotId]

                            return (
                              <div key={slotId} className="border border-gray-200 rounded-lg p-3 sm:p-4 bg-gray-50">
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 sm:gap-3">
                                      <Checkbox
                                        ...
                                        onCheckedChange={(checked) => handleTimeSlotToggle(day, slotId, checked)}
                                        className="w-4 h-4 sm:w-5 sm:h-5"
                                      />
                                      <div>
                                        <Label
                                        ...
                                        <p className="text-xs sm:text-sm text-gray-500">{slotConfig.description}</p>
                                      </div>
                                    </div>
                                  </div>

                                  {slotData?.available && (
                                    /* Made time inputs stack on mobile for better usability */
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                                      <div className="flex items-center gap-2">
                                        <Label className="text-xs sm:text-sm text-gray-600 min-w-[3rem]">Start</Label>
                                        <Input
                                          type="time"
                                          value={slotData?.startTime || slotConfig.defaultStart}
                                          onChange={(e) =>
                                            handleTimeSlotChange(day, slotId, "startTime", e.target.value)
                                          }
                                          className="w-28 sm:w-24 border-gray-300 focus:ring-[hsl(var(--primary-200))] focus:border-[hsl(var(--primary-500))] text-sm h-9 sm:h-10"
                                        />
                                      </div>
                                      <div className="flex items-center gap-2">
                                        <Label className="text-xs sm:text-sm text-gray-600 min-w-[2.5rem]">End</Label>
                                        <Input
                                          type="time"
