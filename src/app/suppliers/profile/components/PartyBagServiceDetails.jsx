...
const PartyBagsServiceDetails = ({ serviceDetails, onUpdate, saving, supplierData, currentBusiness }) => {
  ...
  // Data options
  const themeOptions = [
    'Superhero', 'Princess', 'Unicorn', 'Dinosaur', 'Space', 'Mermaid', 
    'Pirate', 'Fairy', 'Animals', 'Cars', 'Football', 'Rainbow',
    'Jungle Safari', 'Under the Sea', 'Frozen', 'Harry Potter',
    'Minecraft', 'LOL Surprise', 'Paw Patrol', 'General/Mixed'
  ];

  const ageGroupOptions = [
    '0-2 years', '3-5 years', '6-8 years', '9-12 years', '13+ years', 'All ages'
  ];

  const deliveryOptions = [
    'Local Delivery', 'Collection Only', 'UK Wide Shipping', 'Express Delivery'
  ];
  ...
  return (
    <div className="space-y-8">
        ...
        <CardContent className="p-8 space-y-6">
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-600">
              💡 <strong>Tip:</strong> Share what makes your party bags special - quality, themes, personalization, or your passion for making parties memorable!
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="p-8 space-y-8">
          {details.packageTypes.map((pkg, index) => (
            <div key={pkg.id} className="border-2 border-gray-200 rounded-xl p-6 space-y-6">
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                  pkg.id === 'basic' ? 'bg-green-500' : 
                  pkg.id === 'premium' ? 'bg-blue-500' : 'bg-purple-500'
                } text-white`}>
                  {pkg.id === 'basic' ? '1' : pkg.id === 'premium' ? '2' : '3'}
                </div>
                <h3 className="text-lg font-semibold capitalize">{pkg.id} Package</h3>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  onChange={(e) => handlePackageChange(index, 'minimumQuantity', parseInt(e.target.value))}
                  className="h-12 bg-white border-2 border-gray-200 rounded-xl text-base"
                  placeholder="10"
                />
              </div>
        ...
        <CardContent className="p-8">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {themeOptions.map(theme => (
              <div key={theme} className="flex items-center space-x-3 p-4 bg-gray-50 rounded-xl hover:bg-orange-50 transition-colors">
                <Checkbox
                  id={`theme-${theme}`}
        ...
        <CardContent className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={option.key} className="flex items-center space-x-3 p-4 bg-gray-50 rounded-xl hover:bg-green-50 transition-colors">
                <Checkbox
                  ...
                  className="w-5 h-5"
                />
                <Label htmlFor={`custom-${option.key}`} className="text-base font-medium cursor-pointer flex-1">
                  {option.label}
                </Label>
              </div>
            ))}
      ...
      <Card>
        ...
        <CardContent className="p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-center space-x-3 p-4 bg-gray-50 rounded-xl hover:bg-indigo-50 transition-colors">
              <Checkbox
                ...
                className="w-5 h-5"
              />
              <Label htmlFor="collection-available" className="text-base font-medium cursor-pointer flex-1">
            ...
            <div className="flex items-center space-x-3 p-4 bg-gray-50 rounded-xl hover:bg-indigo-50 transition-colors">
              <Checkbox
                ...
                className="w-5 h-5"
              />
              <Label htmlFor="local-delivery" className="text-base font-medium cursor-pointer flex-1">
          ...
          {details.delivery?.localDelivery && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-indigo-50 rounded-xl">
              <div className="space-y-3">
                <Label className="text-base font-semibold text-gray-700">
                ...
                <Input
                  ...
                  onChange={(e) => handleNestedFieldChange('delivery', 'deliveryRadius', parseInt(e.target.value))}
                  className="h-12 bg-white border-2 border-gray-200 rounded-xl text-base"
                  placeholder="10"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  onChange={(e) => handleNestedFieldChange('delivery', 'deliveryFee', parseFloat(e.target.value))}
                  className="h-12 bg-white border-2 border-gray-200 rounded-xl text-base"
                  placeholder="5.00"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  onChange={(e) => handleNestedFieldChange('delivery', 'freeDeliveryThreshold', parseInt(e.target.value))}
                  className="h-12 bg-white border-2 border-gray-200 rounded-xl text-base"
                  placeholder="50"
                />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              <Input
                ...
                onChange={(e) => handleNestedFieldChange('policies', 'minimumOrder', parseInt(e.target.value))}
                className="h-12 bg-white border-2 border-gray-200 rounded-xl text-base"
                placeholder="10"
              />
            </div>
            ...
            <div className="space-y-3">
              ...
              <Input
                ...
                onChange={(e) => handleNestedFieldChange('policies', 'advanceNotice', parseInt(e.target.value))}
                className="h-12 bg-white border-2 border-gray-200 rounded-xl text-base"
                placeholder="7"
              />
            </div>
