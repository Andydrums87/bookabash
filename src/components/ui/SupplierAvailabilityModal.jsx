// components/ui/SupplierAvailabilityModal.jsx - Super Simple Version
"use client"

import { useState, useEffect } from "react"
import { AlertTriangle, XCircle, RefreshCw, Loader2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import { UniversalModal, ModalHeader, ModalContent, ModalFooter } from "@/components/ui/UniversalModal"
import { partyBuilderBackend } from "@/utils/partyBuilderBackend"
import { suppliersAPI } from '@/utils/mockBackend'
import { getDateStringForComparison } from '@/utils/dateHelpers'


const SupplierAvailabilityModal = ({ 
  isOpen, 
  onClose, 
  onConfirm, 
  currentDetails, 
  ...
  const checkSuppliers = () => {
    ...
    for (const [type, supplier] of Object.entries(suppliers)) {
      ...
      const timeSlot = newDetails.startTime ? 
        (parseInt(newDetails.startTime.split(':')[0]) < 13 ? 'morning' : 'afternoon') : 
        'afternoon'
  ...
  // In your SupplierAvailabilityModal.jsx, replace the handleFindReplacements function:
...
const handleFindReplacements = async () => {
    ...
    try {
      ...
      for (const unavailableItem of unavailableSuppliers) {
        ...
        const categorySuppliers = allSuppliers.filter(s => 
          s.category === getSupplierCategory(type) && 
          s.id !== unavailableSupplier.id // Exclude the unavailable one
        )
  ...
  return (
    <UniversalModal isOpen={isOpen} onClose={onClose} size="md" theme="warning">
      <ModalHeader 
        title="Oops! Some suppliers aren't free"
        subtitle="A couple of your suppliers can't make the new date"
        theme="warning"
      ...
      <ModalContent className="space-y-4">
        ...
        {!isChecking && !isRebuilding && unavailableSuppliers.length > 0 && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-medium text-red-900">Sorry, these aren't available:</h4>
            </div>
        ...
        {!isChecking && !isRebuilding && unavailableSuppliers.length === 0 && (
          <div className="text-center py-4">
            <p className="text-green-700">Perfect! Everyone's available.</p>
          </div>
        )}
      </ModalContent>
      ...
      <ModalFooter>
        <div className="flex gap-3 w-full">
          ...
          {unavailableSuppliers.length === 0 ? (
            <Button 
              onClick={() => { onConfirm(newDetails); onClose() }}
              className="flex-1"
            >
              Save Changes
            </Button>
