...
// Calculate supplier price from various data locations
...
const getAverageRating = (supplier) => {
  if (supplier.rating) return { rating: supplier.rating, count: supplier.reviewCount || 0 }
...
export default function SupplierBrowseCard({
  supplier,
  ...
  const getImages = () => {
    const images = []
    const coverPhoto = supplier.coverPhoto || supplier.image || supplier.imageUrl || supplier.data?.coverPhoto
    if (coverPhoto) images.push(coverPhoto)

    const gallery = supplier.gallery ||
                    supplier.photos ||
                    supplier.data?.gallery ||
                    supplier.data?.photos ||
                    supplier.serviceDetails?.gallery ||
                    supplier.serviceDetails?.photos ||
                    supplier.serviceDetails?.portfolioImages ||
                    supplier.data?.serviceDetails?.portfolioImages ||
                    []
    ...
    // Also check package images
    const packages = supplier.packages || supplier.data?.packages || []
    packages.forEach(pkg => {
      const pkgImg = typeof pkg.image === 'object' ? pkg.image?.src : pkg.image
      if (pkgImg && !images.includes(pkgImg)) {
        images.push(pkgImg)
      }
    })
  ...
  const name = supplier.name || supplier.businessName || supplier.data?.name || "Unnamed Supplier"
  const price = calculateSupplierPrice(supplier)
  const ratingData = getAverageRating(supplier)
    ...
    >
      ...
      <div className="space-y-1 pt-2">
        ...
        {supplier.googleRating ? (
          <GoogleRatingBadge
            rating={supplier.googleRating}
            reviewCount={supplier.googleReviewCount}
            size="sm"
          />
        ) : ratingData && (
